...
// Rest of the functions definition go in here
   ...
   public static void viewRooms(Hotel esql) {
   	try {
		...
		String query =String.format ("SELECT roomNumber FROM roombookings WHERE hotelID = '%d' AND bookingDate ='%s'", hotelID,day);
		//String query = ("SELECT * FROM roombookings");
		List<List<String>> result = esql.executeQueryAndReturnResult(query); // if this is empty just print unavailable 
		if(result.isEmpty()){
			System.out.println("Unavailable!");
		}else{
			String r = result.get(0).get(0);
			System.out.println(r);
			int room =Integer.parseInt(r);
			String query1 = String.format("SELECT price FROM rooms WHERE hotelID = '%d' AND roomNumber ='%d'",hotelID,room);
			List<List<String>> result1 = esql.executeQueryAndReturnResult(query1);
			String p = result1.get(0).get(0);
			System.out.println("ROOM# "+room+ " is available on "+day+ " for $"+p+".");
